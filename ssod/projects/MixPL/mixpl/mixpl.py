...
class MixPL(SemiBaseDetector):
    ...
    # inputs_list：输入图片批次
    # batch_data_samples：inputs_list对应的图片信息
    ...
    # 这个函数应该就是实现了图像增强操作（包括resize、翻转和擦除）
    ...
    def resize(self, inputs, data_samples, img_shape):
        ...
        # 'homography_matrix'这是个什么东西，还是不太懂，感觉应该是计算标注变换的矩阵
    ...
    def erase(self, inputs, data_samples):
        def _get_patches(img_shape):
            ...
            for _ in range(n_patches):
                ...
                ratio1 = np.random.random() * \
                        (self.semi_train_cfg.erase_ratio[1] - self.semi_train_cfg.erase_ratio[0]) + \
                        self.semi_train_cfg.erase_ratio[0]
                if a == 0:
                    ph, pw = int(img_shape[0] * ratio), int(img_shape[1] * ratio)
                else:
                    ph, pw = int(img_shape[0] * ratio), int(img_shape[1] * ratio1)
        ...
        # 增加一个开关
        p =  np.random.random()
        if not self.semi_train_cfg.erase or p<0.4:
            return inputs, data_samples
    ...
    def mixup1(self, dst_inputs_list, batch_dst_data_samples, src_inputs_list, batch_src_data_samples, scale_interval = [0.5, 1]):
        batch_size = len(dst_inputs_list)
        mixup_inputs_list, batch_mixup_data_samples = [], []
        for i in range(batch_size):
            # src是从cache中得到的
            ...
            src_inputs, src_data_samples = src_inputs_list[i], batch_src_data_samples[i]
            # 尺度缩放
            scale = np.random.uniform(low=scale_interval[0], high=scale_interval[1], size=None)
            # print(type(src_data_samples.img_shape))
            img_shape = (scale*src_data_samples.img_shape[0],scale*src_data_samples.img_shape[1])
            src_inputs, src_data_samples = self.resize(src_inputs, src_data_samples, img_shape)
