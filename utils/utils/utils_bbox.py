...
from nets.rpn_training_ori import AnchorTargetCreator
...
class DecodeRPNBox():
    def __init__(self, std, num_classes):
        self.std            = std
        self.num_classes    = num_classes + 1    
    #   这个函数的作用是把bbox也调整到和resize之前的图像对应。
    def frcnn_correct_boxes(self, box_xy, box_wh, input_shape, image_shape):
        #-----------------------------------------------------------------#
        #   把y轴放前面是因为方便预测框和图像的宽高进行相乘
        #-----------------------------------------------------------------#
        ...
        boxes *= np.concatenate([image_shape, image_shape], axis=-1)
        return boxes
    #roi_scores是softmax之后的前景概率，在0-1之间
    def forward(self, rois, roi_scores, image_shape, input_shape, nms_iou = 0.7, confidence = 0.5):
        bs      = len(rois)
        results = []
        #--------------------------------#
        #   batch_size, num_rois, 4
        #--------------------------------#
        rois        = rois.contiguous().view((bs, -1, 4))  
        roi_scores  = roi_scores.contiguous().view((bs, -1, 1))       
        #----------------------------------------------------------------------------------------------------------------#
        #   对每一张图片进行处理，由于在predict.py的时候，我们只输入一张图片，所以for i in range(len(mbox_loc))只进行一次
        #----------------------------------------------------------------------------------------------------------------#
        for i in range(bs):
            #-------------------------------------------------------------#
            #   利用rpn网络得到的roi，用左下角右上角的形式表示
            #   roi_score是每个roi的前景得分概率
            #-------------------------------------------------------------#
            roi         = rois[i].view((-1, 4))
            rois[..., [0, 2]] = (rois[..., [0, 2]]) / input_shape[1]
            rois[..., [1, 3]] = (rois[..., [1, 3]]) / input_shape[0]
            roi_score   = roi_scores[i].squeeze()
            results.append([])
            confs_m   = roi_score > confidence
            
            if len(roi_score[confs_m]) > 0:
                roi         = roi[confs_m]
                roi_score   = roi_score[confs_m]
                ...
                roi  = roi[keep]
                roi_score   = roi_score[keep].unsqueeze(1)
                c_pred      = torch.cat((roi, roi_score), dim=1).cpu().numpy()
                # 添加进result里
                results[-1].extend(c_pred)
