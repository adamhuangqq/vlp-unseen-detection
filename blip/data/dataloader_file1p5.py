...
class BLIPDataset1p5(Dataset):
    ...
    def get_crop_img_data(self, file, input_shape, confidence = 0.0):
        ...
        for x in ann:
            if float(x[1]) > confidence:
                x_ann = [int(j) for j in x[2:]]
                x_ann_xyhw = [(x_ann[2]+x_ann[0])/2.0, (x_ann[3]+x_ann[1])/2.0, 1.5*(x_ann[2]-x_ann[0]), 1.5*(x_ann[3]-x_ann[1])]
                x_ann_xyxy = [x_ann_xyhw[0]-x_ann_xyhw[2]/2.0, x_ann_xyhw[1]-x_ann_xyhw[3]/2.0, x_ann_xyhw[0]+x_ann_xyhw[2]/2.0, x_ann_xyhw[1]+x_ann_xyhw[3]/2.0, ]
                ann_list.append(x_ann_xyxy)
...
def frcnn_dataset_collate1p5(batch):
    crops = []
    confs = []
    boxes = []
    file_name_list = []
